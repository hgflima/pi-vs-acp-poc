...
export function RulesManager() {
  ...
  const loadItems = useCallback(async () => {
    setLoading(true)
    try {
      const data = await fetchHarnessItems("rules")
      setItems(data)
      setError(null)
    } catch (err) {
...
function RuleListItem({
  ...
  const handleDelete = useCallback(async () => {
    setDeleting(true)
    try {
      await deleteHarnessItem("rules", item.name)
      setDeleteOpen(false)
      onDeleted()
    } catch {
...
function RuleForm({
  ...
  const handleSave = useCallback(async () => {
    ...
    setError(null)
    try {
      await saveHarnessItem("rules", name.trim(), content)
      onSaved()
    } catch (err) {
    ...
    <div className="space-y-4 mt-4">
      ...
      <div className="space-y-3">
        <div>
          ...
          <Input className="mt-1" placeholder="my-rule" value={name} onChange={(e) => setName(e.target.value)} />
        </div>
        <div>
          <Label>Content</Label>
          <div className="mt-1">
...
function RuleEditor({
  ...
  const handleSave = useCallback(async () => {
    ...
    setError(null)
    try {
      await saveHarnessItem("rules", item.name, content)
      onSaved()
    } catch (err) {
      setError(err instanceof Error ? err.message : "Failed to save")
    } finally {
