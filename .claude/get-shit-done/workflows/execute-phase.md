...
```bash
INIT=$(node "/Users/henriquelima/Documents/dev/personal/pi-ai-poc/.claude/get-shit-done/bin/gsd-tools.cjs" init execute-phase "${PHASE_ARG}")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS=$(node "/Users/henriquelima/Documents/dev/personal/pi-ai-poc/.claude/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-executor 2>/dev/null)
```
...
if [[ ! "$ARGUMENTS" =~ --auto ]]; then
  node "/Users/henriquelima/Documents/dev/personal/pi-ai-poc/.claude/get-shit-done/bin/gsd-tools.cjs" config-set workflow._auto_chain_active false 2>/dev/null
fi
```
</step>
...
2. For each plan (sequentially, ignoring wave grouping):
   ...
   c. **If "Execute":** Read and follow `/Users/henriquelima/Documents/dev/personal/pi-ai-poc/.claude/get-shit-done/workflows/execute-plan.md` **inline**
...
```bash
node "/Users/henriquelima/Documents/dev/personal/pi-ai-poc/.claude/get-shit-done/bin/gsd-tools.cjs" state begin-phase --phase "${PHASE_NUMBER}" --name "${PHASE_NAME}" --plans "${PLAN_COUNT}"
```
...
```bash
PLAN_INDEX=$(node "/Users/henriquelima/Documents/dev/personal/pi-ai-poc/.claude/get-shit-done/bin/gsd-tools.cjs" phase-plan-index "${PHASE_NUMBER}")
```
...
2. **Spawn executor agents:**
   ...
   Task(
     ...
     prompt="
       ...
       @/Users/henriquelima/Documents/dev/personal/pi-ai-poc/.claude/get-shit-done/references/checkpoints.md
       @/Users/henriquelima/Documents/dev/personal/pi-ai-poc/.claude/get-shit-done/references/tdd.md
       </execution_context>
...
5b. **Pre-wave dependency check (waves 2+ only):**
    ...
    ```bash
    node "/Users/henriquelima/Documents/dev/personal/pi-ai-poc/.claude/get-shit-done/bin/gsd-tools.cjs" verify key-links {phase_dir}/{plan}-PLAN.md
    ```
...
```bash
AUTO_CHAIN=$(node "/Users/henriquelima/Documents/dev/personal/pi-ai-poc/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow._auto_chain_active 2>/dev/null || echo "false")
AUTO_CFG=$(node "/Users/henriquelima/Documents/dev/personal/pi-ai-poc/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.auto_advance 2>/dev/null || echo "false")
```
...
```bash
POST_PLAN_INDEX=$(node "/Users/henriquelima/Documents/dev/personal/pi-ai-poc/.claude/get-shit-done/bin/gsd-tools.cjs" phase-plan-index "${PHASE_NUMBER}")
```
...
```bash
PARENT_INFO=$(node "/Users/henriquelima/Documents/dev/personal/pi-ai-poc/.claude/get-shit-done/bin/gsd-tools.cjs" find-phase "${PARENT_PHASE}" --raw)
...
```bash
node "/Users/henriquelima/Documents/dev/personal/pi-ai-poc/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(phase-${PARENT_PHASE}): resolve UAT gaps and debug sessions after ${PHASE_NUMBER} gap closure" --files .planning/phases/*${PARENT_PHASE}*/*-UAT.md .planning/debug/resolved/*.md
```
</step>
...
```bash
VERIFIER_SKILLS=$(node "/Users/henriquelima/Documents/dev/personal/pi-ai-poc/.claude/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-verifier 2>/dev/null)
```
...
Commit the file:
```bash
node "/Users/henriquelima/Documents/dev/personal/pi-ai-poc/.claude/get-shit-done/bin/gsd-tools.cjs" commit "test({phase_num}): persist human verification items as UAT" --files "{phase_dir}/{phase_num}-HUMAN-UAT.md"
```
...
```bash
COMPLETION=$(node "/Users/henriquelima/Documents/dev/personal/pi-ai-poc/.claude/get-shit-done/bin/gsd-tools.cjs" phase complete "${PHASE_NUMBER}")
```
...
```bash
node "/Users/henriquelima/Documents/dev/personal/pi-ai-poc/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(phase-{X}): complete phase execution" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md {phase_dir}/*-VERIFICATION.md
```
</step>
...
```bash
node "/Users/henriquelima/Documents/dev/personal/pi-ai-poc/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(phase-{X}): evolve PROJECT.md after phase completion" --files .planning/PROJECT.md
```
...
Read and follow `/Users/henriquelima/Documents/dev/personal/pi-ai-poc/.claude/get-shit-done/workflows/transition.md`, passing through the `--auto` flag so it propagates to the next phase invocation.
